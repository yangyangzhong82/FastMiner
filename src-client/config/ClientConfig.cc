...
void ClientConfig::buildDefaultConfig() {
    data.blocks.clear();
    data.blocks = {
        // clang-format off
        // 树木类
...
std::shared_ptr<RuntimeBlockConfig> ClientConfig::buildRuntimeBlockConfig(BlockConfig const& config) {
    ...
    for (auto& block : config.similarBlock) {
        rtConfig->similarBlock_.emplace(getBlockIdCached(block));
    }
    return rtConfig;
}
